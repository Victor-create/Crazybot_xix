...
double b=0;  //posicion inicial en 0
double b1=0; // Posicion incial en 0
...
int main(int argc, char **argv)
{
  ...
  double a, a1;//posiciones actuales
  double left_RPM, right_RPM;
  
  ////rueda izquierda//////////////
  a = wb_position_sensor_get_value(ps_left);
          pos_final = ((a - b)*1)/0.064;  //0-064 es el time step en segundos
  ...
  float radio=0.075; //radio de la llanta
  double linvel, linvel2;//velocidad lineal por llanta
  double linvel_rob; //velocidad lineal del robot
  
  linvel=pos_final*radio;
  linvel2=pos_final1*radio;
  linvel_rob=(linvel+linvel2)/2;
